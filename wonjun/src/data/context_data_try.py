...
def process_context_data(users, books, ratings1, ratings2):
    ...
    # 인덱싱 처리된 데이터 조인
    context_df = ratings.merge(users, on='user_id', how='left').merge(books[['isbn', 'category_new', 'publisher', 'isbn_country', 'book_author']], on='isbn', how='left')
    train_df = ratings1.merge(users, on='user_id', how='left').merge(books[['isbn', 'category_new', 'publisher', 'isbn_country', 'book_author']], on='isbn', how='left')
    test_df = ratings2.merge(users, on='user_id', how='left').merge(books[['isbn', 'category_new', 'publisher', 'isbn_country', 'book_author']], on='isbn', how='left')

    # 인덱싱 처리
    #loc_city2idx = {v:k for k,v in enumerate(context_df['location_city'].unique())}
    #loc_state2idx = {v:k for k,v in enumerate(context_df['location_state'].unique())}
    loc_country2idx = {v:k for k,v in enumerate(context_df['isbn_country'].unique())}
    ...
    # book 파트 인덱싱
    category2idx = {v:k for k,v in enumerate(context_df['category_new'].unique())}
    publisher2idx = {v:k for k,v in enumerate(context_df['publisher'].unique())}
    language2idx = {v:k for k,v in enumerate(context_df['isbn_country'].unique())}
    author2idx = {v:k for k,v in enumerate(context_df['book_author'].unique())}

    train_df['category_new'] = train_df['category_new'].map(category2idx)
    train_df['publisher'] = train_df['publisher'].map(publisher2idx)
    train_df['isbn_country'] = train_df['isbn_country'].map(language2idx)
    train_df['book_author'] = train_df['book_author'].map(author2idx)
    test_df['category_new'] = test_df['category_new'].map(category2idx)
